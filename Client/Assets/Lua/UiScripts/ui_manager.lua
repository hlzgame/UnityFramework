...
function ui_manager:show_popup(ui_session, is_close_cur, args, uiUtils_handler)
    ...
    if uiUtils_handler and uiUtils_handler.before_handler then
        uiUtils_handler.before_handler()
    end
    ...
    ui_session, function(go)
        ...
        if uiUtils_handler and uiUtils_handler.after_handler then
            uiUtils_handler.afterHandler()
        end
    end )
end

-- 关闭栈顶弹窗
function ui_manager:close_popup(uiUtils_handler)
...
function ui_manager:hide_session(session_id, uiUtils_handler)
    if self._shown_sessions[session_id] == nil then
        return
    end
    self._shown_sessions[session_id]:hide_session(uiUtils_handler)
    self._shown_sessions[session_id] = nil
end

function ui_manager:destroy_session(session_id, uiUtils_handler)
    ...
    self._all_sessions[session_id]:destroy_session(uiUtils_handler)
